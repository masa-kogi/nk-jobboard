class ApplicantsController < ApplicationController
  ...
  before_action :authenticate_admin_or_recruiter!
  before_action :set_instances_for_form, only: [:index, :show, :edit]
  before_action :set_application_statuses, only: [:index, :edit]
  before_action :set_root_departments, only: [:index, :show, :edit]
  ...
  # エラーハンドリング: 期待されるJobApplicationが存在しない場合はエラーメッセージを表示
    unless @job_application
      redirect_to job_applicants_path, alert: '対象の応募情報が見つかりませんでした。'
      return
    end
  ...
  def update
    ...
    if new_status_id == @job_application.current_status.id
      redirect_to job_applicant_path(@job, @job_application.applicant_id), alert: 'ステータスは変更されていません'
      return
    end
    ...
    if new_status_id.present?
      ...
      redirect_to job_applicants_path, notice: '応募状況を更新しました'
    else
      redirect_to job_applicant_path(@job, @job_application.applicant_id), alert: 'ステータスの更新に失敗しました'
    end
  end
  ...
  def authenticate_admin_or_recruiter!
    unless admin_signed_in? || recruiter_signed_in?
      redirect_to new_recruiter_session_path, alert: 'ログインしてください。'
    end
  end
