...
window.onload = function () {
  ...
  let categorySelect = document.querySelector('#job-category-filter-form select[name="category"]');
  ...
  document.querySelectorAll('.department-link').forEach(function (link) {
    link.addEventListener('click', function (e) {
      ...
      // 隠しフィールドに部門IDをセット
      document.querySelector('#job-category-filter-form input[name="department"]').value = clickedDepartmentId;
  ...
  document.querySelector('#job-category-filter-form').addEventListener('change', function () {
    loadJobs();
  });
  ...
  function loadJobs(page) {
    ...
    let departmentValue = document.querySelector('#job-category-filter-form input[name="department"]').value;
    ...
    let queryParams = `/jobs?${params.toString()}`;
